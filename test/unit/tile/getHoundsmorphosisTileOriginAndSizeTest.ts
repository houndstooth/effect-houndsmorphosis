...
describe('get houndsmorphosis tile origin and size', () => {
	...
	describe('if either x or y are 0', () => {
		it('returns undefineds', () => {
			expect(subject({ gridAddress: to.Address([ 0, 1 ]) })).toEqual(undefined)
			expect(subject({ gridAddress: to.Address([ 1, 0 ]) })).toEqual(undefined)
			expect(subject({ gridAddress: to.Address([ 0, -1 ]) })).toEqual(undefined)
			expect(subject({ gridAddress: to.Address([ -1, 0 ]) })).toEqual(undefined)
		})
	})
	...
	describe('when both x and y are positive', () => {
		describe('first row', () => {
			it('grows by 1 each column, starting at 1', () => {
				actual = subject({ gridAddress: to.Address([ 1, 1 ]) })
				actual && expect(actual.tileSize).toEqual(to.Unit(1))
				actual = subject({ gridAddress: to.Address([ 2, 1 ]) })
				actual && expect(actual.tileSize).toEqual(to.Unit(2))
				actual = subject({ gridAddress: to.Address([ 3, 1 ]) })
				actual && expect(actual.tileSize).toEqual(to.Unit(3))
				actual = subject({ gridAddress: to.Address([ 4, 1 ]) })
		...
		describe('second row', () => {
			it('starts and stays at size 1', () => {
				actual = subject({ gridAddress: to.Address([ 1, 2 ]) })
				actual && expect(actual.tileSize).toEqual(to.Unit(1))
				actual = subject({ gridAddress: to.Address([ 2, 2 ]) })
				actual && expect(actual.tileSize).toEqual(to.Unit(1))
				actual = subject({ gridAddress: to.Address([ 3, 2 ]) })
				actual && expect(actual.tileSize).toEqual(to.Unit(1))
				actual = subject({ gridAddress: to.Address([ 4, 2 ]) })
				actual && expect(actual.tileSize).toEqual(to.Unit(1))
			})
		...
		describe('third row', () => {
			it('grows by 1 each column, starting at 2', () => {
				actual = subject({ gridAddress: to.Address([ 1, 3 ]) })
				actual && expect(actual.tileSize).toEqual(to.Unit(2))
				actual = subject({ gridAddress: to.Address([ 2, 3 ]) })
				actual && expect(actual.tileSize).toEqual(to.Unit(3))
				actual = subject({ gridAddress: to.Address([ 3, 3 ]) })
				actual && expect(actual.tileSize).toEqual(to.Unit(4))
				actual = subject({ gridAddress: to.Address([ 4, 3 ]) })
		...
		describe('fourth row', () => {
			it('starts and stays at size 2', () => {
				actual = subject({ gridAddress: to.Address([ 1, 4 ]) })
				actual && expect(actual.tileSize).toEqual(to.Unit(2))
				actual = subject({ gridAddress: to.Address([ 2, 4 ]) })
				actual && expect(actual.tileSize).toEqual(to.Unit(2))
				actual = subject({ gridAddress: to.Address([ 3, 4 ]) })
				actual && expect(actual.tileSize).toEqual(to.Unit(2))
				actual = subject({ gridAddress: to.Address([ 4, 4 ]) })
				actual && expect(actual.tileSize).toEqual(to.Unit(2))
			})
